...
while IFS= read -r line; do
	...
	if echo "${val}" | grep -E "${regexp}" >/dev/null; then
		...
		response=$(curl -sS -k -X POST -H "content-type: application/json" -d "{\"vault_name\": \"${vault_name}\", \"item_title\": \"${vault_item_title}\", \"auth\": \"${OP_AUTH}\"}" "${OP_ENDPOINT}/get-password")
		RESPONSE_ERROR="$(echo "${response}" | grep -o '"error":"[^"]*' | grep -o '[^"]*$')"
		if [ "${RESPONSE_ERROR}" != "" ]; then
			echo "ERROR: ${RESPONSE_ERROR}"
			exit 1
		fi
		RESPONSE_PASSWORD="$(echo "${response}" | grep -o '"password":"[^"]*' | grep -o '[^"]*$')"
